...
try:
    ...
    from ..utils.lora import LinearWithLoRA
except ImportError:
    from quantization import LearnableFakeQuantize
    from utils.lora import LinearWithLoRA
...
class CPTAttention(nn.Module):
    ...
    def set_precision(self, weight_bits, activation_bits, gradient_bits=8, kv_bits=None):
        """Set precision for all components - used during cyclic training.

        Args:
            weight_bits: Bits for weight quantization (cycles with CPT)
            activation_bits: Bits for activation quantization (cycles with CPT)
            gradient_bits: Bits for gradient quantization (fixed, default 8)
            kv_bits: Bits for KV cache quantization
        """
        self.current_bits = weight_bits
        # Note: gradient_bits support needs to be added in LinearWithLoRA
...
class CPTMLP(nn.Module):
    ...
    def set_precision(self, weight_bits, activation_bits, gradient_bits=8):
        """Set precision for all components - used during cyclic training.
...
class CPTBlock(nn.Module):
    ...
    def set_precision(self, attn_bits, mlp_bits, activation_bits, gradient_bits=8, kv_bits=None):
        """Set precision for attention and MLP layers - allows layer-wise precision.

        Args:
            attn_bits: Bits for attention weights
            mlp_bits: Bits for MLP weights
            activation_bits: Bits for activations
            gradient_bits: Bits for gradients (fixed, default 8)
            kv_bits: Bits for KV cache
        """
        self.attn.set_precision(attn_bits, activation_bits, gradient_bits, kv_bits)
        self.mlp.set_precision(mlp_bits, activation_bits, gradient_bits)
    ...
    def set_layer_precision(self, layer_config):
        ...
        for i, config in enumerate(layer_config):
            if i < len(self.h):
                ...
                gradient_bits = config.get('gradient_bits', 8)

                self.h[i].set_precision(attn_bits, mlp_bits, activation_bits, gradient_bits, kv_bits)

    def set_global_precision(self, bits, activation_bits=None, gradient_bits=8):
        """
        Set the same precision for all layers - simple cyclic training mode.

        Args:
            bits: Number of bits to use for weight quantization (cycles with CPT)
            activation_bits: Number of bits for activation quantization (defaults to bits, cycles with CPT)
            gradient_bits: Number of bits for gradient quantization (fixed, default 8)
        """
        ...
        self.current_bits = bits
        for block in self.h:
            block.set_precision(bits, bits, activation_bits, gradient_bits, bits)

    def set_precision(self, weight_bits, activation_bits=None, gradient_bits=8):
        """Alias for set_global_precision for compatibility.

        Args:
            weight_bits: Bits for weights (cycles with CPT)
            activation_bits: Bits for activations (defaults to weight_bits, cycles with CPT)
            gradient_bits: Bits for gradients (fixed, default 8)
        """
        self.set_global_precision(weight_bits, activation_bits, gradient_bits)
