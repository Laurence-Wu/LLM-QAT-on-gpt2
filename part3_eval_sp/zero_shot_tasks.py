...
class ZeroShotEvaluator:
    ...
    def _generate_answer(self, prompt: str, max_length: int) -> str:
        MAX_CONTEXT = 256
        max_generation = min(max_length, 50)
        max_prompt_length = MAX_CONTEXT - max_generation
        inputs = self.tokenizer(prompt, return_tensors='pt', truncation=True, max_length=max_prompt_length, padding=False)
        ...
        with torch.no_grad():
            ...
            outputs = self.model.generate(inputs['input_ids'], max_length=total_length, temperature=gen_cfg['temperature'], do_sample=gen_cfg['do_sample'], eos_token_id=self.tokenizer.eos_token_id, attention_mask=inputs.get('attention_mask', None))
