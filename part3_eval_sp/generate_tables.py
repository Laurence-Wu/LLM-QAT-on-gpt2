...
class ResultTableGenerator:
    ...
    def generate_table_1_zero_shot(self) -> str:
        ...
        for config_name, result in self.results.items():
            ...
            row = {'Method': config_name, '#Bits': result.get('bits', 'N/A'), 'Size(GB)': result.get('model_size_gb', 0)}
    ...
    def generate_table_2_perplexity(self) -> str:
        ...
        for config_name, result in self.results.items():
            ...
            row = {'Method': config_name, '#Bits': result.get('bits', 'N/A'), 'OpenWebText↓': perplexity.get('OpenWebText', float('inf')), 'WikiText2↓': perplexity.get('WikiText2', float('inf'))}
            rows.append(row)
        if not rows:
    ...
    def generate_table_7_few_shot(self) -> str:
        ...
        for config_name, result in self.results.items():
            ...
            row = {'Method': config_name, 'MMLU-Hum': mmlu.get('Humanities', 0), 'MMLU-STEM': mmlu.get('STEM', 0), 'MMLU-Social': mmlu.get('Social Sciences', 0), 'MMLU-Other': mmlu.get('Other', 0), 'MMLU-Avg': mmlu.get('Average', 0), 'TriviaQA': few_shot.get('TriviaQA', 0)}
            rows.append(row)
        if not rows:
    ...
    def _generate_latex_table_1(self) -> str:
        latex = '\\begin{table}[h]' + '\n'
        latex += '\\centering' + '\n'
        latex += '\\caption{Zero-shot Common Sense Performance}' + '\n'
        latex += '\\begin{tabular}{l|c|c|cccccccc|c}' + '\n'
        latex += '\\hline' + '\n'
        latex += 'Method & \\#Bits & Size(GB) & BoolQ & PIQA & SIQA & HellaSwag & WinoGrande & ARC-e & ARC-c & OBQA & Avg \\\\' + '\n'
        latex += '\\hline' + '\n'
        for config_name, result in self.results.items():
    ...
    def _generate_latex_table_2(self) -> str:
        latex = '\\begin{table}[h]' + '\n'
        latex += '\\centering' + '\n'
        latex += '\\caption{Perplexity Results}' + '\n'
        latex += '\\begin{tabular}{l|c|cc}' + '\n'
        latex += '\\hline' + '\n'
        latex += 'Method & \\#Bits & OpenWebText$\\downarrow$ & WikiText2$\\downarrow$ \\\\' + '\n'
        latex += '\\hline' + '\n'
        for config_name, result in self.results.items():
