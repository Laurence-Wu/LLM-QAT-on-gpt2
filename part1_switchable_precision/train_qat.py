...
from shared.memory_monitor import training_step_context, MemoryMonitor
...
def train_qat(model, train_loader, val_loader, config, model_config):
    ...
    memory_monitor = MemoryMonitor(threshold_mb=500, verbose=False)
    ...
    for iteration in progress_bar:
        ...
        if iteration % 50 == 0:
            memory_monitor.check_memory_leak(f"Iteration {iteration}")
...
def evaluate(model, val_loader, device, use_amp):
    ...
    with torch.no_grad():
        with training_step_context():  # Use context manager for eval too
